...
from .models import OrderPaymentTransaction
...
class Payment(viewsets.ViewSet):
    ...
    def initialize_payment(self, request, *args, **kwargs):
        ...
        try:
            ...
            OrderPaymentTransaction.objects.create(
                user=request.user, razorpay_order_id=order["id"], amount=amount
            )

            return JsonResponse(
                {"orderId": order["id"], "currency": currency, "amount": amount},
                status=200,
            )
    ...
    def verify_payment(self, request, *args, **kwargs):
        ...
        try:
            ...
            txn = OrderPaymentTransaction.objects.get(order_id=order_id).first()
            if txn:
            ...
            if txn:
                ...
                txn.save()
            return JsonResponse(
                {"error": "Payment signature verification failed"}, status=400
            )
        except Exception as e:
